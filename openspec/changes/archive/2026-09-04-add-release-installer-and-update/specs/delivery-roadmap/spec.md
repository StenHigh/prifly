...
5. High-priority public release distribution добавляет signed GitLab Release,
   documented user-space bootstrap и explicit `prifly update`; первая
   публикация не объявляется до configured signing key, release assets и
   проверки полного пути установки/обновления.
