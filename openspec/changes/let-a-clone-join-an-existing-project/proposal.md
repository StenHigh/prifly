...
Третье место того же рода нашлось при разборе отчёта и подтверждено двумя
стендами: справка `session task` обещает, что «тот же документ записан в
`task.json` в рабочей папке попытки», и это верно только для формы с одной
попыткой (`session task --run RUN`), которая и есть вручение. Форма `--all`
— перечисление, файла не пишет, а именно её называет сгенерированный
`prifly-run`: «read outstanding handoffs with `session task --run RUN_ID
--all`». То есть обещанного файла не видит никто, кто следует нашему же
раннеру.
...
- Ответ `project start` и `session task` называют рабочую копию абсолютным
  ...
  перебором.
- Обещание про `task.json` и поведение сходятся: либо `--all` вручает каждую
  названную попытку так же, как форма с одной, либо справка перестаёт обещать
  файл там, где его не будет. Выбор — в design.
