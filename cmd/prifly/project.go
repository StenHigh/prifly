...
// projectRunnerSkillTemplateBeforeShortening is the exact 0.13.36 text, kept so
// a runner an earlier release wrote is still recognised as generated.
var projectRunnerSkillTemplateBeforeShortening = strings.NewReplacer(
...
// projectRunnerSkillTemplate is the current instruction. It was rewritten at
// 0.13.37 against measurement rather than taste: over three battle Runs a host
// used two of its eight numbered steps, and the rest described paths that never
// occur under a project with standing answers -- the legacy /2 profile, the
...
that port -- do not pass it again. This host is {{host}}: pass
` + "`--host {{host}}`" + ` only when the contract needs assisted execution or host-bound
...
({{question_tool}}) rather than Markdown buttons, keep the real mutually
...
// projectRunnerSkill renders the current instruction. The three sections that
// used to be appended -- the native question tool, the decision bridge and the
// catalog dialogue -- are part of the template now: appending them separately
...
func projectRunnerSkillBeforeShortening(host projectHost) string {
	...
	return projectRunnerSkillFromTemplate(host, projectRunnerSkillTemplateBeforeShortening, questions) + projectTimedDecisionBridgeInstructions + projectNeutralCatalogInstructions
}
