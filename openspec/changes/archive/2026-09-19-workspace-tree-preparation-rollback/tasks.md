...
- [x] 3.1 Delta-spec `runtime-resources` синхронизирован в основной spec при
  архивации.

- [x] 3.3 Проверено на опубликованных бинарях решающей парой: тот же стенд
  `stand:package/tree-rollback` на 0.13.39 оставляет `.stand/PLAN.md` в
  рабочей папке после отказа допуска, на 0.13.40 — снимает, а файл владельца
  `.stand/NOTES.md` цел в обоих. Без прогона на 0.13.39 чистый каталог
  одинаково читался бы как «откат сработал» и как «шаг не готовился».
