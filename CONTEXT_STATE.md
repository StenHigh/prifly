...
- Ночной автономный прогон `aif-classic` 1.7.1 (отчёт пилота, 2026-09-04) — движковое:
  ответ на решение недоступен по документированному пути. `run decisions` отдаёт
  в `pending` только `definition_digest`, а `--request-digest` требует
  `DecisionRequestDigest(pending)` — это значение печатает лишь `data.request_digest`
  события `decision.requested` (`run events`). Хуже: `local.Receipt` сериализует
  digest полезной нагрузки команды под тем же именем `request_digest`
  ([store.go:135](internal/local/store.go:135), [store.go:268](internal/local/store.go:268)),
  поэтому квитанция `run decision request` подсовывает похожее, но неверное значение.
  Отказ один на три причины (чужой decision, чужая версия, чужой digest) и не называет
  ни ожидаемое, ни полученное ([main.go:796](cmd/prifly/main.go:796)). Пилот получил
  четыре отказа подряд. Не регрессия этого релиза; `run-decision-ledger/1` не входит в
  pinned bundles, так что дополнительное поле возможно — но это нормативная правка
  cli-protocol и требует отдельного change.
- Оттуда же, продуктовое: под политикой `autonomous` решение с `automatic: false`
  паркует Run в `waiting_decision`, `session submit` закрывается, и отвечать некому —
  ночной заход не доезжает до цикла гейт→починка→гейт. Исполнитель ответил сам
  (`none`), то есть `sensitivity: scope-changing` в autonomous остаётся
  рекомендацией, а не границей: тот же OS-principal владеет обоими каналами. Это
  согласуется с trust model F1, но должно быть сказано явно. Материал для
  `add-run-decision-catalog`.
- Оттуда же, требует проверки: мост разогрева не объявляет формат байтов для
  `outputs/handoff`, и схема отправки его, по наблюдению пилота, не проверяет —
  неверный формат прошёл бы молча.
- Замер выжимки (тот, о котором договаривались): корпус 919 687 знаков, прочитано
  ≈125 000, `handoff` 10 518 — сжатие ≈12×, а не 58×. Из прочитанного 44 500 знаков
  ушло на обвязку самого Pri-Fly (задание, `context.json`, мост, навык, схемы), то есть
  больше трети — на понимание инструмента, а не проекта. Вывод про форк сессии не
  меняется: артефакт дешевле.
- Остальное из отчёта — в `prifly-aif-workflows` (мост планирования молчит про вход
  `handoff` и теряет объявленный `gate_warnings`; выжимка не перечисляет проектные
  оверлеи навыков, включая MANDATORY `skill-context/aif-plan/SKILL.md`; половина
  ссылочных файлов нерелевантна без `+check`). Сюда не тянуть.
