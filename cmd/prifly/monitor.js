...
function activityRows(run) {const rows=[];for(const a of values(run.attempts).filter(a=>!a.settled)){const r=executionRole(a);rows.push({node:a.id,title:r.kind,detail:r.detail+' · '+r.state});}for(const a of values(run.activations).filter(a=>a.status==='ready'))rows.push({node:a.id,title:'Готово к выдаче',detail:a.stage_id});if(run.pending_decision)rows.push({node:run.pending_decision.attempt_id,title:'Ожидается решение',detail:run.pending_decision.decision_id});for(const w of values(run.wait_registrations).filter(w=>w.status==='active'))rows.push({node:w.activation_id,title:'Ожидается сигнал',detail:w.target_stage_id});if(run.fork)rows.push({run:run.fork.source_run_id,title:run.fork.reason==='project continuation'?'Продолжение от Run':'Run создан как fork',detail:run.fork.source_run_id});return rows;}
