...
Уточнение метода при реализации: `testing/synctest` с настоящим native
process и Unix listener не продвигает process-observation timer, пока listener
остаётся вне durable blocking; единый mixed-тест остановлен по лимиту 60 s.
Production clock seam ради теста не добавляется. Доказательство разделено:
runtime synctest проверяет именно две недели pending question, reopen,
остаток 50 минут, приём exact artifact той же Attempt и отказ истёкшего claim;
public CLI mixed отдельно исполняет обе настоящие программы и timed bridge
с exact отчётом Total 48. Ещё один runtime regression держит две недели
уже отвеченную Attempt в ожидании допуска. Ни snapshot, ни timestamps
записанного Run не редактируются; native mixed с двумя неделями времени
...
Владелец подтвердил включение этой защиты 2026-09-06. В scope входит durable
исключительное использование claim конкретным Run до безопасного завершения
или явного освобождения, с атомарной проверкой при выдаче работы и без
автоматического снятия по lease. Предыдущая остановка снята; capability пока
...
Использовать существующие ClaimRecord и coupled authority/Run transaction:
versioned claim↔Run binding проверяется вместе с первым допуском. Тот же Run
повторно использует своё владение; другой Run не наследует его по свободному
slot или окончанию lease. Новая admission legacy шага тоже проверяет текущее
владение: сохранённые старые bytes и сроки не меняются, но старый reader не
даёт bypass текущей защиты. Уже выданная legacy delivery не отбирается и не
приписывается новому Run; неоднозначные держатели требуют явного recovery.

Освобождение ресурса сохраняет прежнюю explicit операцию. При незавершённом
или uncertain владельце она отказывает; проверенный terminal Run допускает
release. Сначала durable fence запрещает дальнейшую выдачу, затем cleanup
вне write-транзакции; прерванный cleanup остаётся видимым и повторяемым без
выдуманного освобождения. Checkout cleanup не меняет файлы. Новый daemon,
очередь claim и автоматическое продление lease не добавляются.
Release удерживает существующий authority-wide driver lock до конца операции:
cleanup не пересекается с подготовкой файлов до атомарного admission.
