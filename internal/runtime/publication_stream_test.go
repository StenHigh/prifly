...
func TestEachPublicationKeepsIndependentCursorsAndPendingAssignments(t *testing.T) {
	...
	r := driverRun(t, e, runID)
	if r.SchemaVersion != CoreMaterializedStateVersion || len(r.PublicationSubscriptions) != 2 {
	...
	for name, value := range map[string]any{
		"CoreRunStateV30": r, "CoreRunViewV30": view, "CoreNextViewV30": next, "CorePreviewV30": preview,
		"CoreWorkflowInvocationV30": r.Invocations[r.RootInvocationID], "CoreCapabilitiesV30": Capabilities(),
	} {
...
func TestNewOnlyStreamStartsAfterItsAuthorityCut(t *testing.T) {
	...
	if err := validatePublic(t, "CoreRunStateV30", r); err != nil {
...
func TestEachPublicationInterruptsOnTerminalProducerFailure(t *testing.T) {
	...
	r := driverRun(t, e, started.Receipt.RunID)
	if r.SchemaVersion != CoreMaterializedStateVersion || len(r.PublicationAssignments) != 2 {
