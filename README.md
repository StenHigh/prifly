...
напрямую в shell. Первая установка доверяет HTTPS asset официального GitHub
Release; subsequent `prifly update` проверяет подпись release перед заменой
binary. Скрипт не меняет shell profile: если понадобится, он подскажет, как
...
| **Различает исходы** | успешный, неудачный и неопределённый; повтор не считается безопасным по умолчанию |
