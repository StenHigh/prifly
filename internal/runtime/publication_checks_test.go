...
func TestArtifactPublicationChecksAcceptSealedItemBeforeProducerSettles(t *testing.T) {
	...
	for name, value := range map[string]any{"CoreRunStateV15": r, "CoreRunViewV15": runView, "CoreNextViewV15": next, "ArtifactPublication": r.ArtifactPublications[0], "CoreCapabilitiesV35": Capabilities()} {
		if err := validatePublic(t, name, value); err != nil {
