...
func TestUntimedAssistedDecisionAcceptsLateAnswerButRejectsResult(t *testing.T) {
	...
	synctest.Test(t, func(t *testing.T) {
		...
		if before.SchemaVersion != "core-state/30" || task.SchemaVersion != "assisted-session/7" || attempt.Session.Timing != nil {
...
func TestLegacyAssistedRunsCannotShareInstallationClaim(t *testing.T) {
	...
	if second := driverRun(t, e, started.Receipt.RunID); len(second.Attempts) != 0 || second.SchemaVersion != "core-state/30" {
