...
```sh
prifly monitor
# Ограничить фоновый поиск выбранными каталогами (macOS/Linux):
...
проектов и worktree. В пользовательском config-каталоге `Pri-Fly/monitor/sources`
хранятся только пути к authority; сами Run и артефакты остаются на месте.
Старые хранилища обнаруживаются фоновым обходом доступных локальных каталогов.
Системные и сетевые файловые системы исключаются; символические ссылки при
обходе не раскрываются, их физические каталоги проверяются по исходным путям.
