...
Размещение, сделанное подготовкой, MUST быть возвращаемым до тех пор, пока
шаг не допущен: если команда, допускающая шаг, отказывает, runtime MUST снять
ровно то, что положила подготовка, и вернуть рабочую папку в состояние, в
котором её застал. Ресурс, через который снимается размещение, MUST жить не
меньше самого отката. Откат MUST NOT трогать то, чего не клал: каталог,
содержащий чужие файлы, остаётся. Пути, которые вернуть не удалось, MUST быть
названы в ответе на этот отказ, а не отброшены: откат, который молчит,
читается одинаково и когда снял всё, и когда не снял ничего.
...
#### Scenario: Допуск отказан после подготовки дерева
- **WHEN** подготовка положила declared input entries в claimed Workspace, а
  команда, допускающая шаг, отказала
- **THEN** положенные подготовкой entries сняты, рабочая папка владельца не
  содержит файлов, которых он не просил, и его собственные файлы в тех же
  каталогах остались нетронутыми

#### Scenario: Откат не смог снять положенное
- **WHEN** откат после отказа не смог удалить часть положенного
- **THEN** ответ называет эти пути вместе с причиной отказа, а код самого
  отказа остаётся прежним
...
Нормативный source set — `openspec/specs/runtime-resources/`.
