...
func TestOncePublicationInterruptsOnTerminalProducerFailure(t *testing.T) {
	...
	if before.SchemaVersion != CoreMaterializedStateVersion || len(before.Active) != 1 {
	...
	if err := validatePublic(t, "CoreRunStateV30", after); err != nil {
...
func TestNewOnlyOncePublicationRejectsItemsBeforeItsAuthorityCut(t *testing.T) {
	...
	r := driverRun(t, e, runID)
	if r.SchemaVersion != CoreMaterializedStateVersion {
