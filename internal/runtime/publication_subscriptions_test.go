...
func TestOncePublicationInterruptsOnTerminalProducerFailure(t *testing.T) {
	...
	if before.SchemaVersion != CoreEffectsStateVersion || len(before.Active) != 1 {
	...
	if err := validatePublic(t, "CoreRunStateV29", after); err != nil {
...
func TestNewOnlyOncePublicationRejectsItemsBeforeItsAuthorityCut(t *testing.T) {
	...
	r := driverRun(t, e, runID)
	if r.SchemaVersion != CoreEffectsStateVersion {
