...
- `run drive` получает границу: вести Run через управляющие стадии и вручения,
  но **остановиться перед исполнением программы** и вернуть управление. Замер
  боевого захода на 0.13.32: `run next` за секунду до вызова показал
  `{"action":"stage","stage_id":"decide","stage_work":"control"}` — честно, —
  а тот же `run drive` прошёл управляющую стадию насквозь и исполнил
  двенадцатиминутный прогон тестов внутри себя. Поле `stage_work`, выпущенное
  в 0.13.32, называет работу видимой стадии, а программа стояла за ней;
  предупреждения не вышло именно там, ради чего поле делалось.
- `project runners update` получает проверку без записи (`--check`): в клоне,
  где launch объявляет предстартовую программу «дерево должно быть чистым»,
  штатная замена раннера делает следующий `project start` невозможным до
  коммита, а узнать об этом заранее нечем. Просьба холодного захода; там это
  стоило отдельного разбора, почему старт вдруг отказывает.
